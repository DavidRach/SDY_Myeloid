...
if (stringr::str_detect(Experiment, "AB_")){
    files <- files[stringr::str_detect(files, "-Cord|ND006|-00_|-00")]
}
...
TheData <- purrr::map(.x=MyGatingSet, Wetlab_Concentration, subset = "CD45",
               nameKeyword=nameKeyword, DilutionMultiplier=100, TotalVolume=1) |> dplyr::bind_rows()
...
CD45 <- purrr::map(.x=MyGatingSet, Wetlab_Concentration, subset = "CD45",
               nameKeyword=nameKeyword, DilutionMultiplier=100, TotalVolume=1) |> dplyr::bind_rows() |> dplyr::select(name, ConcentrationScientific)
